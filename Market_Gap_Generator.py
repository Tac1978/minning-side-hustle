#!/usr/bin/env python3
"""
Market Gap Generator (Step 5 & End-to-End Pipeline)

Brings all framework steps together:
Step 1: Market & Niche Segmentation (segmentation.py)
Step 2: Reddit Query Formatting (reddit_query.py)
Step 3: Pain Point Extractor Prompt (Pain_Point_Extractor.py)
Step 4/5: Market Gap Generator Prompt (Market_Gap_Generator.py)
"""
...
from Pain_Point_Extractor import format_pain_point_extractor_prompt

MARKET_GAP_GENERATOR_TEMPLATE = """Market Gap Generator
Context
...
def format_market_gap_generator_prompt(pain_points_summary: str) -> str:
    """
    Formats the Market Gap Generator prompt template with the extracted pain points summary.
    """
    if not pain_points_summary or not pain_points_summary.strip():
        raise ValueError("Pain points summary must not be empty.")

    return MARKET_GAP_GENERATOR_TEMPLATE.format(
        pain_points_summary=pain_points_summary.strip()
    )


def run_framework_pipeline(segment_query: str, reddit_data: str = "") -> Dict[str, Any]:
    """
    Brings all framework steps together for a given segment query.
    Returns a dictionary containing outputs for all steps in the pipeline.
    """
    if not segment_query or not segment_query.strip():
        raise ValueError("Segment query must not be empty.")

    segmentation_output = get_segmentation_output(segment_query)
    reddit_query_str = format_reddit_query(segment_query)

    extractor_prompt = None
    if reddit_data and reddit_data.strip():
        extractor_prompt = format_pain_point_extractor_prompt(reddit_data)

    return {
        "segment_query": segment_query,
        "segmentation_output": segmentation_output,
        "reddit_query": reddit_query_str,
        "pain_point_extractor_prompt": extractor_prompt,
    }


def main():
    parser = argparse.ArgumentParser(
        description="Market Gap Generator prompt generator & end-to-end framework pipeline"
    )
    parser.add_argument(
        "-p", "--pain-points",
        type=str,
        help="Pain points summary text or file path to format Market Gap Generator prompt"
    )
    parser.add_argument(
        "-s", "--segment",
        type=str,
        help="Run end-to-end pipeline for the given market segment"
    )
    parser.add_argument(
        "-d", "--data",
        type=str,
        help="Optional Reddit data text or file path to include in pipeline execution"
    )

    args = parser.parse_args()

    if args.pain_points:
        points_text = args.pain_points
        if os.path.exists(args.pain_points):
            with open(args.pain_points, "r", encoding="utf-8") as f:
                points_text = f.read()
        print(format_market_gap_generator_prompt(points_text))
    elif args.segment:
        reddit_text = ""
        if args.data:
            reddit_text = args.data
            if os.path.exists(args.data):
                with open(args.data, "r", encoding="utf-8") as f:
                    reddit_text = f.read()

        results = run_framework_pipeline(args.segment, reddit_text)
        print("=== STEP 1: SEGMENTATION ===")
        print(results["segmentation_output"])
        print("\n=== STEP 2: REDDIT SEARCH QUERY ===")
        print(results["reddit_query"])
        if results["pain_point_extractor_prompt"]:
            print("\n=== STEP 3: PAIN POINT EXTRACTOR PROMPT ===")
            print(results["pain_point_extractor_prompt"])
    else:
        parser.print_help()
